...
const SSAOShader = {
	...
	fragmentShader: /* glsl */`
		...
		float getDepth( const in vec2 screenPosition ) {
			...
			return pow(texture2D( tDepth, screenPosition ).x, 4.);
		...
		void main() {
			...
			for (int x = -2; x < 2; x++){
				for (int y = -2; y < 2; y++){
					float sample_depth = getDepth(vec2(
						vUv.x + float(x) * (texel_size.x * 25.),
						vUv.y + float(y) * (texel_size.y * 25.)
					));
					
					occlusion += clamp(depth- sample_depth, 0., 1.);
				}
			}
